...
function InquiryStatusEditor({ inquiry, onClose }: { inquiry: ContactInquiry | null; onClose: () => void }) {
  ...
  const save = useMutation({ mutationFn: () => api.patch(`/contact/inquiries/${inquiry?.id}/status`, { id: inquiry?.id, status, adminNotes: adminNotes || null }), onSuccess: () => { toast.success("Inquiry updated."); void queryClient.invalidateQueries(); onClose(); }, onError: (error) => toast.error(error instanceof Error ? error.message : "Update failed.") });
...
function BookingStatusEditor({ booking, onClose }: { booking: BookingRequest | null; onClose: () => void }) {
  ...
  const save = useMutation({ mutationFn: () => api.patch(`/contact/bookings/${booking?.id}/status`, { id: booking?.id, status, adminNotes: adminNotes || null }), onSuccess: () => { toast.success("Booking updated."); void queryClient.invalidateQueries(); onClose(); }, onError: (error) => toast.error(error instanceof Error ? error.message : "Update failed.") });
